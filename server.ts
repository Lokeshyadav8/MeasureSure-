...
// ============================================================================
// STATUTORY DATABASE AUTHENTICATION & ENCRYPTED HASH STORAGE (PBKDF2-SHA512)
// ============================================================================

// Get safe user registry from persistent database
app.get("/api/auth/users", (_req, res) => {
  try {
    const users = getAllSafeUsers();
    res.json({ users });
  } catch (err: any) {
    console.error("[Auth API Error] Failed to retrieve users:", err);
    res.status(500).json({ error: "Failed to fetch registered users from database" });
  }
});

// Register user into database with unique 16-byte cryptographic salt and PBKDF2-SHA512 hash
app.post("/api/auth/register", (req, res) => {
  try {
    const { name, email, password, phone, role, businessOrDepartment, licenseNumber } = req.body;
    if (!name || !email || !password || !phone) {
      return res.status(400).json({ error: "Name, email, contact phone, and password are required." });
    }
    ...
    const result = registerNewUser({
      ...
      businessOrDepartment: businessOrDepartment || '',
    ...
    // Also remember user contact/email upon registration
    saveRememberedUser({
      email,
      phone,
      role: role || 'BUSINESS_OWNER'
    ...
    console.error("[Auth API Error] Registration failed:", err);
    return res.status(500).json({ error: err?.message || "Registration failed" });
  }
});

// Authenticate user against irreversible salted hash in database
app.post("/api/auth/login", (req, res) => {
  try {
    const { emailOrId, password, role, rememberMe, phone } = req.body;
    if (!emailOrId) {
      return res.status(400).json({ error: "Email, badge ID, or license number is required." });
    }

    const result = authenticateUser(emailOrId, password || '', role);
    if (!result.success) {
    ...
    console.error("[Auth API Error] Login failed:", err);
    return res.status(500).json({ error: err?.message || "Authentication failed" });
  }
});

// Get remembered user email, phone, and role (password is NEVER stored in remembered file)
app.get("/api/auth/remembered", (_req, res) => {
  try {
    const remembered = getRememberedUser();
    res.json({ remembered });
  } catch (err: any) {
    res.json({ remembered: null });
  }
});
...
app.post("/api/auth/remember", (req, res) => {
  try {
    const { email, phone, role, clear } = req.body;
    if (clear) {
      clearRememberedUser();
      return res.json({ success: true, cleared: true });
    }
    saveRememberedUser({ email, phone, role });
    return res.json({ success: true, remembered: { email, phone, role } });
  } catch (err: any) {
    return res.status(500).json({ error: "Failed to update remembered user state" });
  }
});

// Security Audit & Breach Protection Transparency Report
app.get("/api/auth/security-audit", (_req, res) => {
  try {
